...
def get_raw_scores(examples, preds):
    ...
    for example in examples:
        qas_id = int(example.unique_id)
        gold_answers = normalize_answer(example.span_text)
...
def span_detection_evaluate(examples, preds, output_best_answers_file, three_way=False):
    unique_id_pred = {example.unique_id: example.pred for example in examples}
    if three_way:
        ...
        #exact, f1 = get_raw_scores_v2(examples, preds)
        exact, f1, best_answers = get_raw_scores(examples, preds)
...
def get_final_text(pred_text, orig_text, do_lower_case, verbose_logging=False):
    ...
    a = tokenizer.tokenize(orig_text)
            ...
            for i in range(top_n):
                for j in range(top_n):
                    ...
                    if length > max_answer_length:
                        continue
                    min_answer_length = min_answer_length if min_answer_length else None
...
#                 print(best_non_null_entry)
        ...
        scores_diff_json[example.unique_id] = score_diff
        #print(scores_diff_json[example.unique_id.item()])
        # note(zhiliny): always predict best_non_null_entry
        # and the evaluation script will search for the best threshold
        all_predictions[example.unique_id] = answers
        all_nbest_json[example.unique_id] = nbest_json
...
def compute_predictions_log_probs_bs1(
    all_examples,
    all_features,
    all_results,
    n_best_size,
    ...
    example_index_to_features = collections.defaultdict(list)
    for feature in all_features:
        example_index_to_features[feature[0].example_index].append(feature[0])

    unique_id_to_result = {}
    for result in all_results:
        unique_id_to_result[result.unique_id.item()] = result
